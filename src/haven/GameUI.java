...
public class GameUI extends ConsoleHost implements Console.Directory, UI.Notice.Handler {
    ...
    public GameUI(String chrid, long plid, String genus) {
        ...
        mapmenupanel = add(new Hidepanel("mapmenu", new Indir<Coord>() {
            public Coord get() {
                return (new Coord(0,
                        Math.min(blpanel.c.y - mapmenupanel.sz.y + UI.scale(33),
                                GameUI.this.sz.y - mapmenupanel.sz.y)));
            }
        }, new Coord(-1, 0)));
        ...
        menupanel = add(new Hidepanel("menu", new Indir<Coord>() {
            public Coord get() {
                return (new Coord(GameUI.this.sz.x,
                        Math.min(brpanel.c.y - UI.scale(79), GameUI.this.sz.y - menupanel.sz.y)));
            }
        }, new Coord(1, 0)));
        ...
        buffs = ulpanel.add(new Bufflist(), portrait.c.x + portrait.sz.x + UI.scale(10),
                portrait.c.y + ((IMeter.fsz.y + UI.scale(2)) * 2) + UI.scale(5 - 2));
        calendar = umpanel.add(new Cal(), Coord.z);
        eqproxyHandBelt = add(new EquipProxy(CFG.UI_SHOW_EQPROXY_HAND, SLOTS.HAND_LEFT, SLOTS.HAND_RIGHT, SLOTS.BELT),
                UI.scale(420, 5));
        eqproxyPouchBack = add(
                new EquipProxy(CFG.UI_SHOW_EQPROXY_POUCH, "EquipProxy2", SLOTS.POUCH_LEFT, SLOTS.POUCH_RIGHT,
                        SLOTS.BACK),
                UI.scale(420, 35));
    ...
    private void closeWindows() {
        ...
        closeWindow(actlist);
        actlist = null;
        closeWindow(craftlist);
        craftlist = null;
        closeWindow(buildlist);
        buildlist = null;
        closeWindow(studywnd);
        studywnd = null;
    }
    ...
    public void toggleActList() {
        if (actlist == null) {
            actlist = add(
                    new ActWindow("Act…",
                            "paginae/act/.+|paginae/pose/.+|paginae/gov/.+|paginae/add/.+|gfx/fx/msrad|ui/tt/q/quality"),
                    ClientUtils.getScreenCenter(ui));
    ...
    public void addchild(Widget child, Object... args) {
        ...
        if (place == "mapview") {
            ...
            updhand();
            synchronized (heldNotifier) {
                heldNotifier.notifyAll();
            }
        } else if (place == "chr") {
    ...
    public static class GobTrans implements Window.Transition<GobTrans.Anim, GobTrans.Anim> {
        ...
        public Anim show(Window wnd, Anim hide) {
            return (new Anim(wnd, false, hide));
        }

        public Anim hide(Window wnd, Anim show) {
            return (new Anim(wnd, true, show));
        }
    }
    ...
    public static class Progress extends Widget {
        ...
        public void set(double prog) {
            ...
            img.getGraphics().drawImage(txt, (img.getWidth() - txt.getWidth()) / 2, UI.scale(8) - txt.getHeight() / 2,
                    null);
    ...
    public class MainMenu extends Widget {
        public MainMenu() {
            super(menubg.sz());
            add(new MenuCheckBox("rbtn-inv", kb_inv, "Inventory"), 0, 0).state(() -> wndstate(invwnd))
                    .click(() -> togglewnd(invwnd));
            add(new MenuCheckBox("rbtn-equ", kb_equ, "Equipment"), 0, 0).state(() -> wndstate(equwnd))
                    .click(() -> togglewnd(equwnd));
            add(new MenuCheckBox("rbtn-chr", kb_chr, "Character Sheet"), 0, 0).state(() -> wndstate(chrwdg))
                    .click(() -> togglewnd(chrwdg));
            add(new MenuCheckBox("rbtn-bud", kb_bud, "Kith & Kin"), 0, 0).state(() -> wndstate(zerg))
                    .click(() -> togglewnd(zerg));
            add(new MenuCheckBox("rbtn-opt", kb_opt, "Options"), 0, 0).state(() -> wndstate(opts))
                    .click(() -> togglewnd(opts));
        }
    ...
    public class MapMenu extends Widget {
        ...
        public MapMenu() {
            super(mapmenubg.sz());
            add(new MenuCheckBox("lbtn-claim", kb_claim, "Display personal claims"), 0, 0)
            ...
            add(new MenuCheckBox("lbtn-ico", kb_ico, "Icon settings"), 0, 0).state(() -> wndstate(iconwnd))
                    .click(() -> {
                        if (iconconf == null)
                            return;
                        if (iconwnd == null) {
        ...
        public void draw(GOut g) {
            for (int i = 0; i < 12; i++) {
                ...
                try {
                    if (belt[slot] != null)
                        belt[slot].draw(
                                g.reclip(c.add(UI.scale(1), UI.scale(1)), invsq.sz().sub(UI.scale(2), UI.scale(2))));
                } catch (Loading e) {
                }
        ...
        public NKeyBelt() {
            ...
            adda(new IButton("gfx/hud/hb-btn-chat", "", "-d", "-h") {
                Tex glow;
                {
                    ...
                    glow = new TexI(
                            PUtils.rasterimg(PUtils.blurmask(up.getRaster(), UI.scale(2), UI.scale(2), Color.WHITE)));
                }
        ...
        cmdmap.put("farmtest", new Console.Command() {
            public void run(Console cons, String[] args) {
                ...
                try {
                    ...
                    cons.out.println("Error: Coordinates must be numbers");
                }
            }
        });
        
        cmdmap.put("farmtestcircle", new Console.Command() {
            public void run(Console cons, String[] args) {
                if (args.length < 4) {
                    cons.out.println("Usage: farmtestcircle <gridX> <gridY> <radius> [name]");
                    cons.out.println("Creates a circular test field at the specified grid coordinates");
                    return;
                }
                try {
                    double gridX = Double.parseDouble(args[1]);
                    ...
                    String name = args.length > 4 ? args[4] : "Test Circle";
                    auto.farming.util.FarmingTestUtils.createTestCircleField(gridX, gridY, radius, name);
                    cons.out.println("Created circular field '" + name + "' at grid (" + gridX + ", " + gridY + ") radius " + radius);
                } catch (NumberFormatException e) {
                    cons.out.println("Error: Coordinates and radius must be numbers");
                }
            }
        });
        
        cmdmap.put("farmclear", new Console.Command() {
            public void run(Console cons, String[] args) {
